...
async def fetch_pgscatalog_traits(
    url="https://www.pgscatalog.org/rest/trait/all", efo_terms=None
):
...
def main():
    ...
    flat = [(value, key) for key, values in disease_tuples for value in values]
    ...
    with open("pgscatalog.csv", "w", newline="\n") as csvfile:
        ...
        for subject, object in flat:
            writer.writerow(
                {
                    "subject": subject,
                    "predicate": "predicts_disposition",
                    "object": object,
                }
            )
